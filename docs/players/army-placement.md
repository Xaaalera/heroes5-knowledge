...
Разбор относится к исследованной сборке **Heroes V: Повелители Орды с Universe**. Он объединяет чтение кода игры, проверки отдельных участков и наблюдения на тестовой карте и запись попыток размещения. Это не полная спецификация всех режимов: границы подтверждения указаны [в конце статьи](#evidence). 
...
Скрипт рассчитывает позиции из маски и характеристик примера; записанные итоговые клетки используются только для последующей сверки. Оценки силы в скрипте вычисляются из характеристик четырёх существ, а не задаются готовым порядком. Он проверяет два поля, а для `elementals_trace` — ещё и точное совпадение журнала попыток. Защитные/разреженные режимы, другие составы и запасные ветки этим примером не реализованы.

## Проверка на другом поле: почему огонь сменил ряд

Теперь тот же состав стоит на поле с другой маской препятствий. В ряду Y=2 подход свободен на 10 клеток, а в Y=3 преграда оставляет только одну. Минимум переместился **с ряда 2 в ряд 3** — огненный элементаль получает `(13,3)`.
...
У нестрелков первые три подходящих ряда остались **8, 4, 10**. Расчёт даёт земле `(12,8)`, воде `(12,4)`, воздуху `(12,10)` — все четыре позиции совпали с записью Start этого боя. В отличие от предыдущего опыта, внутренний журнал попыток здесь не снимался.
...
*Огонь стоит в `(13,3)`. У земли, воды и воздуха сохранились ряды 8, 4 и 10: изменение препятствий в этом примере поменяло первое место стрелка, но не первые подходящие места остальных отрядов.*
